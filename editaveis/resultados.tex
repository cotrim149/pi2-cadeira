...
Nesse esquemático pode ser observado que a interface com o usuário recebe um input do usuário, utilizando Joystick ou Aplicativo. O sinal emitido por esta interface será processado em um \textit{Raspeberry Pi}, que em seguida irá movimentar o motor.
...
O acionamento dos transistores é feito a partir de uma onda quadrada, o PWM (\textit{Pulse Width Modulation}). O PWM é basicamente um onda com vários pulsos que possuem a sua largura controlada, figura \ref{fig:ondas_quadradas}, que é chamado de \texit{Duty Cycle}, essa largura determina o tempo com o qual os transistores ficarão ligados, controlando assim a passagem de corrente e a velocidade dos motores.
...
O Raspberry Pi tem como principal componente um pequeno circuito integrado que reúne o processador com a arquitetura ARM, a GPU VideoCore IV e a memória RAM que é compativel com o sistema operacional GNU/Linux. As especificações gerais do modelo mais provável a ser utilizado no projeto, o \texit{Raspberry Pi Model B}, que pode ser visto na figura \ref{fig:rasp}, são:
