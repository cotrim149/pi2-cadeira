...
\section{Controle}
  ...
  O controle da cadeira de rodas foi feito utilizando 3 módulos: Joystick, Controlador central e Motor.
...
Panororama do sistema de controle conforme Arquitetura
  - Desde controle de \textit{software} a periféricos (Usuário a Motor)
...
Joystick
  - \textit{software} e Hardware (periféricos e MSP Joystick)
...
O distribuidor é responsável por alimentar os motores elétricos e os módulos de controle dos movimentos da cadeira de rodas. Ele contará com um bloco de entrada que estará conectado diretamente nos terminais da bateria, onde a partir desse bloco de entrada teremos duas saídas de distribuição, uma com 12 volts e outra com 5 volts.

A saída de distribuição de 12 volts foi realizada através de cabos que foram conectados nos terminais dos motores. Essa saída é dupla, ou seja, cada motor tem sua alimentação individual, sendo que para proteção dos dois motores, utilizaremos fuzíveis inseridos antes dos plugs dos moteres, para que caso ocorra alguma variação de corrente, os motores não sejam danificados.

A saída de distribuição de 5 volts é proveniente da utilização de um regulador de tensão 7805. São utilizados diodos zeners(falar qual) que garantem que a tensão se mantenha estável nos 5 volts. Essa saída é responsável pela alimentação das duas pontes H, da Raspberry Pi, dos 2 msp430 e do joystick.
